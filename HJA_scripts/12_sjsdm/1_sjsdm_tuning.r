...
## Make grid of priority tune parameters, from these in sampling; then add lower priority parameters
...
tune.rows = sample(1:nrow(tune.grid), size = nstep, replace = FALSE)
...
tune.results = data.frame(tr = 1:nstep,
                           tune.grid[tune.rows,], 
                           lambda.bio = sample(sample.bio, size = nstep, replace = TRUE),
                           alpha.bio = sample(sample.bio, size = nstep, replace = TRUE),
                           loglike_sjSDM = NA, 
                           loss= NA, 
...
tune.results = cbind(tune.results[rep(seq(nstep), k),], k = rep(1:k, each = nstep))
...
#	iter = 17L
	iter = 170L; otu.train = otu.pa.train} else stop("check abund")
...
for (i in seq_len(k)){
	...
	a = scale.dd(t.otu.train, t.otu.valid)
	t.otu.train = a[1]; t.otu.valid = a[2]
...
#		cat("\nk", i, "tune run", j)
		...
		## SAve each model
    # saveRDS(model.train,
    #         file.path(modpath,'tuning', paste0("s-jSDM_tuning_model_", varsName, "_", k, "CV_", i, "_tr_", j, "_", abund, ".rds")))
		...
		for (pred in c('train','valid')) {
			# pred -> test (valid); training
			if (pred=='valid') {newdd = t.env.valid; newsp = t.XY.valid; otudd = t.otu.valid}
			if (pred=='train') {newdd = NULL; newsp = NULL; otudd = t.otu.train}
...
#				tune.results$auc.lt5.train[tune.results$k == i][j]  = sum(auc < 0.5, na.rm = T)
			...
			if (pred=='valid') {
				tune.results$AUC.valid[tune.results$k == i][j] = mean(auc, na.rm = TRUE)
