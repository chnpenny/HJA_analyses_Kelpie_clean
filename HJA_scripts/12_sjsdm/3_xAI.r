...
tuning = read.table(here(modpath, 'tuning', glue('best_manual_tuning_sjsdm_{sjsdmV}_{varsName}_{cv}_
		 {period}_{abund}_min{minocc}_nSteps{nstep}.csv')), header = T, sep = ',') 
...
spp.list = taxadd[c(sample(1:10,1), sample.int((nrow(taxadd)*.45):(nrow(taxadd)*.55),1), 
				  sample.int((nrow(taxadd)*.9):nrow(taxadd),1 )), ]  
spp.list
