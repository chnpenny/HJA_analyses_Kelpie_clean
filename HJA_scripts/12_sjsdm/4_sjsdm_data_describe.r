...
# model setting info
load(here('source', glue('filtered_info_{cv}_{period}_random_min{minocc}_{date.model.run}_{varsName}.rdata')))
	
# ..... load OTU data ......
alldata = read.csv(here(otupath, glue('sample_by_species_table_{samtoolsfilter}_minimap2_{minimaprundate}_kelpie{kelpierundate}_FSL_qp.csv')), header=T, sep=',', stringsAsFactors = F, na.strings='NA')
dim(alldata)
...
names(otu.pa.train) == names(otu.pa.test)
...
#           , output_file = here(despath, glue('heat-map_{tdraw}_{varsName}_{sepe}_{abund}_min{minocc}.pdf'))
) 
...
p1 = ggplot(exp, aes(UTM_E, UTM_N),shape=assign) + geom_point(aes(colour=assign,shape=assign)) + scale_shape_manual(values=c(19,3)) + scale_colour_manual(values=c('lightgreen','chocolate1'))
